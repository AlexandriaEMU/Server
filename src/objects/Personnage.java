...
public class Personnage {
	...
	public Personnage(int _guid, String _name, int _sexe, int _classe, int _color1, int _color2, int _color3,int _lvl, int _size, int _gfxid, Map<Integer,Integer> stats, String stuff,int pdvPer, byte seeAlign, int mount, int alvl, byte alignement)
	{
		...
		if(!stuff.equals(""))
		{
			if(stuff.charAt(stuff.length()-1) == '|')
				stuff = stuff.substring(0,stuff.length()-1);
			//SQLManager.LOAD_ITEMS(stuff.replace("|",",")); Osef d�j� charger ?
		}
		for(String item : stuff.split("\\|"))
		{
			if(item.equals(""))continue;
	...
	public String getAsPacket()
	{
		...
		ASData.append(_baseStats.getEffect(Constants.STATS_ADD_PERDOM)).append(",").append(getStuffStats().getEffect(Constants.STATS_ADD_PERDOM)).append(",").append(getDonsStats().getEffect(Constants.STATS_ADD_PERDOM)).append(",").append(getBuffsStats().getEffect(Constants.STATS_ADD_PERDOM)).append("|");
	...
	public void removeByTemplateID(int tID, int count)
	{
		...
		ArrayList<Objet> remove = new ArrayList<>();
		int tempCount = count;
	...
	public void set_curCell(Case cell)
	{
		_curCell = cell;
	}
	...
	public void teleport(short newMapID, int newCellID)
	{
		...
		if(_curCarte.getMountPark() != null && _curCarte.getMountPark().get_owner() > 0 && _curCarte.getMountPark().get_guild().get_id() != -1)
		{
			if(World.getGuild(_curCarte.getMountPark().get_guild().get_id()) == null)//Ne devrait pas arriver
			{
				...
				MountPark.removeMountPark(_curCarte.getMountPark().get_guild().get_id());
			}
		}
		...
		if(get_compte().get_subscriber() == 0 && _curCarte.getSubArea().get_subscribe() && get_compte().get_subscriberMessage() && Main.USE_SUBSCRIBE)///Non abonn�, zone abo et premier message
		{
	...
	private Stats getBuffsStats()
	{
		...
		if(_fight != null)
		{
			...
			if(fighter != null)
			{
				ArrayList<SpellEffect> fightBuffs = new ArrayList<>();
	...
	*/
	...
	public String parseBankPacket()
	{
		...
		if (get_compte().GetBankKamas() != 0)
			packet.append("G").append(get_compte().GetBankKamas());
		return packet.toString();
	}
