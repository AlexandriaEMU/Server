...
public class Objet {
	...
	public static class ObjTemplate
	{
		...
		private ArrayList<Action> onUseActions = new ArrayList<>();
		...
		public Objet createNewItem(int qua,boolean useMax)
		{
			...
			if(type == Constants.ITEM_TYPE_FAMILIER)
			{
				...
				item = new Objet(id, ID, qua, Constants.ITEM_POS_NO_EQUIPED, new Stats(false, null), new ArrayList<>(), new TreeMap<>(), World.get_Pets(ID).generateNewtxtStatsForPets());
			...
			{
				item = new Objet(id, ID, qua, Constants.ITEM_POS_NO_EQUIPED, generateNewStatsFromTemplate(StrTemplate,useMax), getEffectTemplate(StrTemplate), new TreeMap<>(), new TreeMap<>());
			}
			return item;
		}
		...
		for(Entry<Integer,String> entry : txtStats.entrySet())
		{
			...
			else if(entry.getKey() == Constants.STATS_PETS_PDV || entry.getKey() == Constants.STATS_PETS_POIDS || entry.getKey() == Constants.STATS_PETS_DATE)
			{
				...
				else
				{
					...
					if(entry.getKey() == Constants.STATS_PETS_POIDS) stats.append(Integer.toHexString(entry.getKey())).append("#").append(Integer.toHexString(p.parse_Corpulence())).append("#").append(p.get_Corpulence() > 0 ? p.parse_Corpulence() : 0).append("#").append(Integer.toHexString(p.parse_Corpulence()));
