...
public class Fight {
	...
	private final Map<Integer, Challenge> _challenges = new TreeMap<>();
	public Fight(int type, int id, Mapa map, Personaje init1, Personaje init2)
	{
		...
		SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init1.get_GUID() + "", init1.get_GUID() + "," + Constants.ETAT_PORTE + ",0");
		SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init1.get_GUID() + "", init1.get_GUID() + "," + Constants.ETAT_PORTEUR + ",0");
		SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init2.get_GUID() + "", init2.get_GUID() + "," + Constants.ETAT_PORTE + ",0");
		SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, init2.get_GUID() + "", init2.get_GUID() + "," + Constants.ETAT_PORTEUR + ",0");
	...
	public Fight(int id, Mapa map, Personaje init1, MobGroup group) {
		_mobGroup = group;
		...
		for (Entry<Integer, Fighter> entry : e) {
			...
			SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, f.getGUID() + "", f.getGUID() + "," + Constants.ETAT_PORTE + ",0");
			SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, f.getGUID() + "", f.getGUID() + "," + Constants.ETAT_PORTEUR + ",0");
	...
	public Fight(int id, Mapa map, Personaje perso, Percepteur perco) {
		...
		SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, perso.get_GUID() + "", perso.get_GUID() + "," + Constants.ETAT_PORTE + ",0");
		SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, perso.get_GUID() + "", perso.get_GUID() + "," + Constants.ETAT_PORTEUR + ",0");
			...
			public void actionPerformed(ActionEvent event) {
				...
				if (Time <= 0) {
					if (perco != null) {
						...
						for (Entry<Integer, Personaje> P : perco.getDefenseFight().entrySet()) {
	...
	private void startFight() {
		...
		if(_type == Constants.FIGHT_TYPE_PVM)
		{
			...
			severalEnnemies = (_team1.size() >= 2);
			severalAllies = (_team0.size() >= 2);
			bothSexes = (hasMale && hasFemale);
			EvenEnnemies = (_team1.size() % 2 == 0);
			MoreEnnemies = (_team1.size() >= _team0.size());
		...
		for(Fighter F : getFighters(3)) {
			...
			if (perso.isOnMount())
				SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 3, 950, perso.get_GUID() + "", perso.get_GUID() + "," + Constants.ETAT_CHEVAUCHANT + ",1");
	...
	public void endTurn()
	{
		...
		try
		{
			...
			for(SpellEffect SE : _ordreJeu.get(_curPlayer).getBuffsByEffectID(131))
			{
				...
				{
					SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 105, _ordreJeu.get(_curPlayer).getGUID() + "", _ordreJeu.get(_curPlayer).getGUID() + "," + _ordreJeu.get(_curPlayer).getBuff(184).getValue());
					dgt = dgt - _ordreJeu.get(_curPlayer).getBuff(184).getValue();//R�duction physique
				}
				if (_ordreJeu.get(_curPlayer).hasBuff(105)) {
					SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 105, _ordreJeu.get(_curPlayer).getGUID() + "", _ordreJeu.get(_curPlayer).getGUID() + "," + _ordreJeu.get(_curPlayer).getBuff(105).getValue());
				...
				dgt = -(dgt);
				SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 100, SE.getCaster().getGUID() + "", _ordreJeu.get(_curPlayer).getGUID() + "," + dgt);
	...
	public void joinFight(Personaje perso, int guid) {
		...
		if(_perco != null)
		{
			for(Personaje z : World.getGuild(_guildID).getMembers()) {
				if (z.isOnline()) {
	...
	public boolean fighterDeplace(Fighter f, GameAction GA)
	{
		...
		if(tacle != null && !f.isState(6))//Tentative de Tacle : Si stabilisation alors pas de tacle possible
		{
			...
			if(!tacle.isEmpty())//Si tous les tacleur ne sont pas stabilis�s
			{
				...
				if(rand > chance)
				{
					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7,GA._id, "104",_ordreJeu.get(_curPlayer).getGUID()+";", "");//Joueur tacl�
        ...
        && po.isState(Constants.ETAT_PORTEUR))
        {
			...
			//si le joueur va bouger
       		if(nextCellID != po.get_fightCell().getID()) {
				...
				SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 950, po.getGUID() + "", po.getGUID() + "," + Constants.ETAT_PORTEUR + ",0");
				SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 950, _ordreJeu.get(_curPlayer).getGUID() + "", _ordreJeu.get(_curPlayer).getGUID() + "," + Constants.ETAT_PORTE + ",0");
			}
      	}
	...
	public int tryCastSpell(Fighter fighter, SortStats Spell, int caseID) {
		...
		if(CanCastSpell(fighter,Spell,Cell, -1))
		{
			...
			if(isEc)
			{
				...
				SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 302, fighter.getGUID() + "", Spell.getSpellID() + "");
			}else
			{
				if((this._type == Constants.FIGHT_TYPE_PVM) && (this._challenges.size() > 0) && !this._ordreJeu.get(this._curPlayer).isInvocation() && !this._ordreJeu.get(this._curPlayer).isDouble() && !this._ordreJeu.get(this._curPlayer).isPerco())
				{
				...
				SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 300, fighter.getGUID() + "", sort);
				if(isCC)
				{
					if(Main.CONFIG_DEBUG) GameServer.addToLog(fighter.getPacketsName()+" Coup critique sur le sort "+Spell.getSpellID());
					SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 301, fighter.getGUID() + "", sort);
				}
			...
			SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 102, fighter.getGUID() + "", fighter.getGUID() + ",-" + Spell.getPACost());
	...
	public void toggleLockSpec(int guid) {
		if ((_init0 != null && _init0.getGUID() == guid) || (_init1 != null && _init1.getGUID() == guid)) {
			specOk = !specOk;
			if (!specOk) {
				for (Entry<Integer, Personaje> spectateur : _spec.entrySet())//Expulsion des spectateurs
				{
					Personaje perso = spectateur.getValue();
	...
	public String GetGE(int win) {
		...
		if(isCapturable)
	    {
		    ...
		    pierrePleine = new PierreAme(World.getNewItemGuid(),1,7010,Constants.ITEM_POS_NO_EQUIPED, pierreStats.toString());	//Cr�e la pierre d'�me
	...
	public void onGK(Personaje perso) {
		if (_curAction.equals("") || _ordreJeu.get(_curPlayer).getGUID() != perso.get_GUID() || _state != Constants.FIGHT_STATE_ACTIVE)
			return;
		if (Main.CONFIG_DEBUG)
			GameServer.addToLog("(" + _curPlayer + ")Fin du deplacement de Fighter ID= " + perso.get_GUID());
		SocketManager.GAME_SEND_GAMEACTION_TO_FIGHT(this, 7, _curAction);
	...
	public void verifIfTeamAllDead() {
		...
		if(team0 || team1 || !verifyStillInFight())
		{
			...
			for(Personaje perso: _spec.values())
			{
				...
				//SocketManager.INSTANCE.GAME_SEND_GV_PACKET(perso);	//Mauvaise ligne apparemment
			...
			for(Fighter F : winTeam)
			{
				...
				/**Familier**/
				if(_type == Constants.FIGHT_TYPE_PVM)//PvM et Joueurs on gagn�s
				{
					Map<Integer, Integer> souls = new TreeMap<>();
	...
	public void onFighterDie(Fighter target, Fighter caster) {
		...
		if (!target.hasLeft() && target.getPersonnage() != null)
			deadList.put(target.getGUID(), target);//on ajoute uniquement les joueurs � la liste des cadavres
		...
		if (target.isState(Constants.ETAT_PORTEUR)) {
			...
			SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 950, f.getGUID() + "", f.getGUID() + "," + Constants.ETAT_PORTE + ",0");
			SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 950, target.getGUID() + "", target.getGUID() + "," + Constants.ETAT_PORTEUR + ",0");
		}
		if((this._type == Constants.FIGHT_TYPE_PVM) && (this._challenges.size() > 0))
		{
			for(Entry<Integer, Challenge> c : this._challenges.entrySet())
		...
		if(target.getTeam() == 0)
		{
			...
			for(Entry<Integer,Fighter> entry : team.entrySet())
			{
				...
				if(entry.getValue().getInvocator().getGUID() == target.getGUID())//si il a �t� invoqu� par le joueur mort
				{
					...
					SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 999, target.getGUID() + "", getGTL());
				}
			}
	...
	public void tryCaC(Personaje perso, int cellID)
	{
        ...
        }else
		{
			...
			{
				SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 305, perso.get_GUID() + "", "");//Echec Critique Cac
			...
			{
				SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 303, perso.get_GUID() + "", cellID + "");
				...
				if(isCC)
				{
					...
					SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 301, perso.get_GUID() + "", "0");
				}
				...
				SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, 102, perso.get_GUID() + "", perso.get_GUID() + ",-" + PACost);
				SocketManager.GAME_SEND_GAF_PACKET_TO_FIGHT(this, 7, 0, perso.get_GUID());
				verifIfTeamAllDead();
			}
		}
						...
						}else if(T == null)//Il leave de son plein gr� donc (T = null)
						{
							...
							if(isValid2)//Soit il a lancer le combat => annulation du combat
							{
								...
								if(_type == Constants.FIGHT_TYPE_PVT)
								{
									...
									for (Personaje z : World.getCarte(_perco.get_mapID()).getPersos()) {
										if (z == null) continue;
										SocketManager.GAME_SEND_MAP_PERCO_GMS_PACKETS(z.get_compte().getGameThread().get_out(), z.get_curCarte());
									}
								}
	...
	public static class Piege {
		...
		public void desappear() {
			...
			SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, team, 999, _caster.getGUID() + "", str.toString());
			str2.append("GDC").append(_cell.getID());
			SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, team, 999, _caster.getGUID() + "", str2.toString());
			if (get_isunHide()) {
				int team2 = _teamUnHide + 1;
				str3.append("GDZ-").append(_cell.getID()).append(";").append(_size).append(";").append(_color);
				SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, team2, 999, _caster.getGUID() + "", str3.toString());
				str4.append("GDC").append(_cell.getID());
				SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, team2, 999, _caster.getGUID() + "", str4.toString());
			}
		}
		...
		public void onTraped(Fighter target) {
			...
			SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 307, target.getGUID() + "", str);
	...
	public static class Fighter {
		...
		private int _team = -2;
		private Case _cell;
		private final ArrayList<SpellEffect> _fightBuffs = new ArrayList<>();
		...
		public Fighter(Fight f, Personaje perso) {
			_fight = f;
			if (perso._isClone) {
				_type = 10;
				_double = perso;
		...
		public void decrementStates() {
			...
			for (Entry<Integer, Integer> e : entries) {
				...
				if (nVal == 0)//ne pas mettre plus petit, -1 = infinie
				{
					...
					SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 950, getGUID() + "", getGUID() + "," + e.getKey() + ",0");
					continue;
				}
		...
		public void refreshfightBuff() {
			...
			ArrayList<SpellEffect> b = new ArrayList<>();
			for (SpellEffect entry : _fightBuffs) {
				if (entry.decrementDuration() != 0)//Si pas fin du buff
				{
					...
					switch (entry.getEffectID()) {
						...
						case 150://Invisibilit�
							SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 150, entry.getCaster().getGUID() + "", getGUID() + ",0");
							break;
						...
						case 950:
							...
							if (id == -1) return;
							setState(id, 0);
							SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 950, entry.getCaster().getGUID() + "", entry.getCaster().getGUID() + "," + id + ",0");
							break;
					}
				}
			}
		...
		public void addBuff(int id, int val, int duration, int turns, boolean debuff, int spellID, String args, Fighter caster) {
			...
			if (Main.CONFIG_DEBUG)
				GameServer.addToLog("Ajout du Buff " + id + " sur le personnage Fighter ID = " + this.getGUID() + " val : " + val + " duration : " + duration + " turns : " + turns + " debuff : " + debuff + " spellid : " + spellID + " args : " + args);
			...
			for (SpellEffect SE : _fightBuffs) {
				...
				switch (SE.getEffectID()) {
					...
					case Constants.STATS_ADD_PA2:
						SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 101, getGUID() + "", getGUID() + ",-" + SE.getValue());
						break;

					case Constants.STATS_ADD_PM:
					case Constants.STATS_ADD_PM2:
						SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 127, getGUID() + "", getGUID() + ",-" + SE.getValue());
						break;
				}
			}
			_fightBuffs.clear();
		...
		public void unHide(int spellid) {
			...
			SocketManager.INSTANCE.GAME_SEND_GA_PACKET_TO_FIGHT(_fight, 7, 150, getGUID() + "", getGUID() + ",0");
