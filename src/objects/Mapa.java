...
public class Mapa {
	...
	private Map<Integer,Fight> 		_fights 		= new TreeMap<>();
	...
	private Map<Integer,ArrayList<Action>> _endFightAction = new TreeMap<>();
	...
	public void spawnNewGroup(boolean timer,int cellID,String groupData,String condition)
	{
		...
		if(Main.CONFIG_DEBUG) GameServer.addToLog("Groupe de monstres ajoutes sur la map: "+_id+" ID: "+_nextObjectID);
		...
		if(!hasEndingFight)
		{
			for(MobGroup group : _mobGroups.values())
			{
				if(Pathfinding.getDistanceBetween(this,caseID,group.getCellID()) <= group.getAggroDistance())//S'il y aggro
				{
					...
					{
						if(perso.get_compte().get_subscriber() == 0 && getSubArea().get_subscribe() && Main.USE_SUBSCRIBE) return;
